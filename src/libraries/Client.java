...
public class Client extends DBConnection {
    ...
    private SimpleDateFormat dateFormat = new SimpleDateFormat("MMMMM dd yyyy");
    ...
    public void DisplayClientInfo() {
     ...
     public void selectACOP() {
        setData(new Vector());
        try {
            super.getConnectToDbHost();
            PreparedStatement stmt = conn.prepareStatement("select * from clients c LEFT JOIN pensions p ON(c.id=p.client_id) where  IF(p.type = 'Surviving Dependent',MONTH(c.spouse_birthdate) =?, MONTH(c.birthdate) =?)");
            stmt.setInt(1, getId());
            stmt.setInt(2, getId());
            ResultSet rs = stmt.executeQuery();
            while (rs.next()) {
                ...
                rows.add(lastnamel);
                rows.add(typel);
                rows.add(dateFormat.format(Date.valueOf(birthdatel)));
                this.getData().add(rows);
            }
            rs.clearWarnings();
            stmt.close();
