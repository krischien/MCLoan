...
//       
    ...
    public void processAdditionalLoan(double pension, double amortization, int term, double deduction, int id) {
        ...
        loanLib.selectAllLoanTransactions();
        int monthToBePaid = 0;
        int monthpaid = 0;
        int monthunpaid = 0;
        for (int a = 0; a <= loanLib.getData().size(); a++) {
            monthToBePaid += Integer.parseInt(loanLib.getData().get(7).toString());
            paymentCollectionLib.setPnNUm(loanLib.getData().get(2).toString());
            paymentCollectionLib.countTransactionByPN();
            monthpaid += paymentCollectionLib.getId();
            monthunpaid = monthToBePaid - monthpaid;
        }
        double monthlyChangel = pension - amortization;
        double totalDeduction = 0.0;
        LocalDate firstDue = LocalDate.parse(loanLib.getLastDue().toString());
        int monthDiff = monthunpaid + term;
...
//       
    ...
    public void process13thMonthLoan(double pension, double amortization, int term, double deduction) {
        ...
        setInterest(amortization * (12-LocalDate.now().getMonthOfYear())*0.03);
        setPrincipal(0.0);
